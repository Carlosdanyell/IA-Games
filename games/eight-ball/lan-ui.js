// O pareamento funciona também em retrato. Os códigos nunca usam innerHTML.
...
export function createLanLobby({ name, onName, onCreate, onJoin, onAnswer, onLeave, onBack }) {
  const root = el('div', null, 'pool-lan');
  root.append(el('p', 'Dois aparelhos, uma mesa. Conectem os dois ao mesmo Wi-Fi e abram o Neon Pool. Não é preciso instalar servidor.'));
  ...
  const back = button('Voltar para a mesa', onBack);
  back.hidden = true;
  ...
  help.append(el('summary', 'Como conectar / Não conectou?'), el('p',
    '1. Quem cria envia o convite. 2. O amigo abre o convite, toca em Gerar resposta e devolve o código. 3. Quem criou cola a resposta e toca em Conectar. A mesa abre nos dois aparelhos.'), el('p',
  ...
  function output(label, token, asLink) {
    ...
    if (navigator.share) actions.append(button(asLink ? 'Compartilhar convite' : 'Compartilhar resposta', async () => {
      try { await navigator.share({ title: 'Neon Pool · partida na mesma rede', text: value }); }
